        ...
        >
          @for (tab of tabs(); track $index; let index = $index) {
            <button
              ...
              [class]="buttonClassesSignal()[index] + ' inline-flex items-center gap-1.5'"
            >
              @if (tab.icon()) {
                <span class="material-symbols-outlined text-base leading-none" aria-hidden="true">{{ tab.icon() }}</span>
              }
              {{ tab.label() }}
            </button>
          }
...
export class ZardTabGroupComponent implements AfterViewInit {
  /** Apenas filhos diretos — evita que abas aninhadas apareçam na barra do grupo pai. */
  private readonly tabComponents = contentChildren(ZardTabComponent, { descendants: false });
  ...
  /** Classes extras no wrapper da navegação (ex.: bordas em toda a largura). */
  readonly zNavWrapperClass = input<string>('');
  /** Classes extras no `<nav role="tablist">` (ex.: remover `border-b` padrão). */
  ...
  protected readonly navGridClasses = computed(() => {
    ...
    const base = this.showArrow() ? twMerge(clsx('grid w-full', gridLayout)) : 'grid w-full';
    return twMerge(base, this.zNavWrapperClass());
  });
