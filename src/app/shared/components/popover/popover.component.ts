...
export class ZardPopoverDirective implements OnInit, OnDestroy {
  ...
  /** Iguala a largura do painel do overlay à do elemento trigger (ex.: combobox). */
  ...
  show() {
    ...
    if (this.zMatchTriggerWidth()) {
      queueMicrotask(() => this.syncOverlayWidth());
    }
  ...
  private syncOverlayWidth(): void {
    ...
    const triggerWidth = this.nativeElement.getBoundingClientRect().width;
    const pane = this.overlayRef.overlayElement;
    this.renderer.setStyle(pane, 'width', `${triggerWidth}px`);
    this.renderer.setStyle(pane, 'max-width', `${triggerWidth}px`);
    this.overlayRef.updatePosition();
  }

  private clearOverlayWidth(): void {
    if (!this.overlayRef) {
      return;
    }
    ...
    this.renderer.removeStyle(pane, 'max-width');
  }
  ...
  private getMatchTriggerWidthPositions(): ConnectedPosition[] {
    ...
    const positions: ConnectedPosition[] = [aligned[placement]];
  ...
  private getPositions(): ConnectedPosition[] {
    if (this.zMatchTriggerWidth()) {
      return this.getMatchTriggerWidthPositions();
    }
